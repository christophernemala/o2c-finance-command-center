...
import { useRealtimeSync } from '../hooks/useRealtimeInvoices'
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Activate realtime sync for the entire app
  useRealtimeSync()
            ...
            <NavLink key={to} to={to} end={to === '/'}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition ${
                  isActive ? 'bg-blue-50 text-f9-blue font-medium' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                }`}
            >
